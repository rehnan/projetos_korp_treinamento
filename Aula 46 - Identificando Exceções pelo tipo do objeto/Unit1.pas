...
begin
  try
    ...
    on E: EConvertError do
      ShowMessage('(Tratando excessão específica de conversão) Tratando Exceção de Erro de conversão '+E.Message);
    on E: EZeroDivide do
      ShowMessage('(Tratando excessão específica de divisão por 0) Não é possível efetuar a divisão por 0! '+E.Message);
    on E: Exception do
      ShowMessage('(Tratando excessão genérica) Um erro não esperado ocorreu: '+E.Message);
  end;
  EdtNumero1.SetFocus;
end;
end.
